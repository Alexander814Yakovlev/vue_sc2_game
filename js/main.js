let app = new Vue({
    ...
    methods: {
        ...
        goToAchivements() {
            if (localStorage.sc2TotalGames > 0) {
                this.showMain = false;
                this.showSocial = false;
    ...
    computed: {
        ...
        showResultRace() {
            return {
                'zerg': Boolean(this.totalGame.zerg),
                'primal': Boolean(this.totalGame.primal),
                'terran': Boolean(this.totalGame.terran),
                'infested': Boolean(this.totalGame.infested),
                'protoss': Boolean(this.totalGame.protoss),
                'taldarim': Boolean(this.totalGame.taldarim),
                'hybrid': Boolean(this.totalGame.hybrid),
            }
        }
    },
});

let audio = new Audio('audio/soundtrack.m4a');
let audioButton = document.querySelector('.btn__sound');
let audioVolume = document.querySelector('.btn__sound i');
...
audioButton.addEventListener('click', function () {
    if (audio.muted) {
        audio.muted = false;
        audioVolume.className = 'fa fa-volume-up';
    } else {
        audio.muted = true;
        audioVolume.className = 'fa fa-volume-off';
    }
});
